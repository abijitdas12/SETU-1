...
for filepath in js_files:
    ...
    # 3. Inject const xe definition using Wh (Axios module object)
    if b'xe.interceptors.request.use' in data and b'const xe=' not in data and b'const xe =' not in data:
        ...
        repl_xe = b'Se=e=>Promise.resolve(e);const xe=Wh.create({baseURL: "https://setu-backend1.onrender.com", headers: { "Content-Type": "application/json" } });xe.interceptors.request.use'
        if target_xe in data:
            data = data.replace(target_xe, repl_xe)
            print(f"Injected Wh.create for const xe in {filepath}")
